...
Usage:
    ...
    python run.py ingest                    # Run ingestion pipeline
...
def print_menu():
    ...
    print("\nChoose an option:")
    print("\n  1. 🧪 Run System Tests")
    print("  2. � Analyze Contract (PDF)")
    print("  3. 📊 Run Ingestion Pipeline")
    print("  4. ⚙️  Run Setup")
    print("  5. ❌ Exit")
    print("\n" + "=" * 80)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "analyze":
            ...
            sys.exit(subprocess.call(f"python -m backend.retrieval_pipeline.main {verbose} {pdf_file}", shell=True))
        
        elif command == "test":
            sys.exit(subprocess.call("python backend/scripts/test_system.py", shell=True))
        
        elif command == "setup":
            sys.exit(subprocess.call("python scripts/setup.py", shell=True))
        
        elif command == "ingest":
            sys.exit(subprocess.call("python backend/scripts/ingest_pipeline.py", shell=True))
    ...
    while True:
        ...
        if choice == "1":
            run_command("python backend/scripts/test_system.py")
            ...
            if os.path.exists(pdf_file):
                ...
                run_command(f"python -m backend.retrieval_pipeline.main {v_flag} {pdf_file}")
            else:
                print(f"\n❌ File not found: {pdf_file}")
                input("\nPress Enter to continue...")
        
        elif choice == "3":
            run_command("python scripts/setup.py")        
        elif choice == "5":
            break        
        elif choice == "4":
